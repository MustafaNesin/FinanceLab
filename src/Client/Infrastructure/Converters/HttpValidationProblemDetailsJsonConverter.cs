// Copied from: https://github.com/dotnet/aspnetcore/blob/cef52ffd679ff3a1966cce3f9782f13496fff672/src/Shared/HttpValidationProblemDetailsJsonConverter.cs.
...
public sealed class HttpValidationProblemDetailsJsonConverter : JsonConverter<HttpValidationProblemDetails>
{
    private static readonly JsonEncodedText Errors = JsonEncodedText.Encode("errors");

    [UnconditionalSuppressMessage("Trimmer", "IL2026",
        Justification =
            "Trimmer does not allow annotating overriden methods with annotations different from the ones in base type.")]
    public override HttpValidationProblemDetails Read(ref Utf8JsonReader reader, Type typeToConvert,
    ...
    [RequiresUnreferencedCode(
        "JSON serialization and deserialization ProblemDetails.Extensions might require types that cannot be statically analyzed. ")]
    public static void WriteProblemDetails(Utf8JsonWriter writer, HttpValidationProblemDetails value,
        JsonSerializerOptions options)
    {
        ...
        static void SerializeErrors(Utf8JsonWriter writer, IDictionary<string, string[]> errors,
            JsonSerializerOptions options)
